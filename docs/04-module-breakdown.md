...
- **Post-launch addition (Phase 15):** counter capture — `GET /customers/lookup?phone=` recognises a returning customer from their mobile number, and the POS captures phone + name inline for a new one (`POST /customers` is find-or-create on phone, so a repeat number never trips the unique constraint). Numbers are normalised on write and lookup (`shared/phone.ts`) so one person typed six different ways stays one record. Adds `marketing_opt_in`/`marketing_consent_at` (migration 0012) captured by an explicit checkbox at the till — consent is never inferred from having a phone number, per TRAI/DND rules and WhatsApp's Business Policy. Receipts can be sent via a `wa.me` deep link. **Not delivered:** bulk/automated promotional broadcasts, which need the WhatsApp Business API and an approved provider; the consent field is the groundwork, not the feature. Covered by `apps/api/test/customer-lookup.test.ts`.
...
- **Delivered:** search-and-add-to-cart against live branch stock, editable per-line qty/price/discount, split payment across multiple modes, hold/resume (cart snapshot as JSONB), checkout via `/sales`. Barcode scanning works the way a keyboard-wedge scanner does everywhere in retail software — it types into the search box and sends Enter — so no separate "scanner mode" was needed. **A scan whose term exactly matches a variant barcode is added straight to the cart** rather than rendering a result list: barcodes are unique per organization so the match is unambiguous, and requiring a click per item would mean taking a hand off the scanner for every scan. The search box holds focus (on mount, after each add, and after checkout) because a scanner types into whatever is focused — if focus drifts, scanning silently does nothing.
