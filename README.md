...
At the till, typing a customer's **mobile number** recognises a returning customer instantly (and shows any outstanding balance); an unknown number lets you capture their name, an optional email, and an explicit opt-in for offers, saved when the sale completes. The payment panel has a **Send bill to customer** group — Print / SMS / WhatsApp / Email. Tick whichever apply *before* checking out and only those fire; nothing is ever sent unless selected, and a customer who'd rather have the bill on their phone can skip paper entirely. The digital channels send a thank-you message linking to a public bill page the customer opens without logging in. Channels are disabled until the matching contact detail is captured. After the sale the same actions stay available for resending, along with a copyable link (SMS/email deep links do nothing on a desktop till with no handler). Server-side automated sending isn't built: that needs an SMS gateway with DLT registration, or SMTP with SPF/DKIM — see `docs/03-database-design.md` §26.
